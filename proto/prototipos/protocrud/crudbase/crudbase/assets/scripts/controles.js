var CONTROLES = (function () {
    ...
    function configurarTabelaOptions(options, seletor) {
        var tabelaSeletor = seletor != undefined ? seletor : 'table.dataTable';
        if ($(tabelaSeletor).length > 0) {

            if($.fn.DataTable.isDataTable(tabelaSeletor))
             $(tabelaSeletor).DataTable().destroy();

            $(tabelaSeletor).DataTable(options);
        }
    }
    ...
    function configurarTabela(callBackNenhumItem, callbackSucesso) {
        ...
        var paginar = true;
        if (tabela.data('datatables-paginar') !== undefined) {
        ...
        var buscar = false;
        if (tabela.data('datatables-buscar') !== undefined) {
    ...
    return {
        ...
        Tabela: {
            ...
            ConfigurarOpcoes: configurarTabelaOptions
        },
        Plugins: {
